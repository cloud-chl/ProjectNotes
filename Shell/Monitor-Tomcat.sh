...
Monitor() {
  ...
  if [ ${TomcatID} ]; then
    TomcatServiceCode=$(curl -o ${TomcatMonitorLog} -s -m 10 --connect-timeout 10 -w %{http_code} -k ${WebUrl})
    if [ ${TomcatServiceCode} -eq 200 ]; then
      echo "[info]页面返回码为${TomcatServiceCode},Tomcat启动成功,测试页面正常......"
    else
      echo "[error]Tomcat页面出错,请注意......状态码为${TomcatServiceCode},错误日志已输出到${TomcatMonitorLog}"
